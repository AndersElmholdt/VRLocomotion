...
void ULocSaver2::BeginPlay()
{
	...
	if (Record)
	{
		...
		GetOwner()->GetWorldTimerManager().SetTimer(LocationSaverHandle, this, &ULocSaver2::RecordCurrentLocation, 1 / float(RecordRate), true);
	}
	else
	{
		ReadFromFile();
		...
		GetOwner()->GetWorldTimerManager().SetTimer(LocationSaverHandle, this, &ULocSaver2::SetCurrentLocation, 1 / (PlaybackRate * PlaybackRateScale), true);
	}
}

// Writes the location data to a file
void ULocSaver2::WriteToFile() const
{
...
void ULocSaver2::RecordCurrentLocation()
{
	Locations.Add(GetAttachParent()->GetComponentLocation());
	Rotations.Add(GetAttachParent()->GetComponentRotation().Quaternion());
	Velocity = GetAttachParent()->GetComponentVelocity();
	VelocityArray.Add(Velocity);
}

// Sets the actors current location based on the Locations array
void ULocSaver2::SetCurrentLocation()
{
	...
	GetAttachParent()->SetWorldLocation(Locations[CurrentTimeIndex], false);

	if (ApplyRotation)
	{
		// Updates the rotation - RotationOffsetQuat is multiplied with the roation value as that is how you add quaternions
		GetAttachParent()->SetWorldRotation(RotationOffsetQuat * Rotations[CurrentTimeIndex]);
	}
...
*/
FVector ULocSaver2::GetVelocity() const
{
	return Velocity;
}

// Sets the velocity variable - FORCEINLINE to improve performance
FORCEINLINE void ULocSaver2::SetVelocity()
{
